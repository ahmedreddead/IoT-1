...
Micropolis_Siren object(26,27,id);
// Date constructor
Micropolis_Siren::Micropolis_Siren(int vcc, int gnd, char* id_input)
{
    Set_volt(vcc,gnd);
    id = id_input;
}
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect("Micropolis_Siren")) {
      ...
      client.subscribe("Micropolis/Siren");
      st = "{id:" + String(id) + ",status:off}";
      const char* str = st.c_str();
      client.publish("Micropolis/Siren", str);
    } else {
 ...
 void callback(char* topic, byte* message, unsigned int length ) {
  ...
  if (String(topic) == "Micropolis/Siren") {
